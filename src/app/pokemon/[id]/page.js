...
async function obterDetalhesPokemon(id) {
  ...
  if (!resposta.ok) {
    throw new Error('Falha ao buscar os dados do Pokemon.');
  }
  
  return resposta.json();
}

export default async function PaginaDetalhes({ params }) {
  const { id } = await params;
  
  // Verificar se é um Pokémon customizado (ID muito grande indica customizado)
  if (id > 10000) {
    return <DetalhesPokemon id={id} />;
  }
  
  const pokemon = await obterDetalhesPokemon(id);
